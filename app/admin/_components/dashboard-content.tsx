      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <Link href="/admin/promotions">
          <StatCard
            ...
            change={`${stats.promotions.total} gesamt`}
            isPositive={true}
        ...
        <Link href="/admin/reviews">
          <StatCard
            title="Bewertungen"
            value={stats.reviews.pending.toString()}
            change="ausstehend"
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-nova-50">
                  {stats.recentOrdersList.map((order) => (
                    <tr
                      key={order.id}
                      className="group cursor-pointer transition-colors hover:bg-nova-50/30"
                    >
                      ...
                      <td className="px-6 py-4 text-sm font-medium text-nova-600">
                        {order.customerName}
                      </td>
                      <td className="px-6 py-4">
                        <span
                          className={`rounded-lg px-2.5 py-0.5 text-xs font-bold ${getStatusColor(order.status)}`}
                        >
                          {getStatusTranslation(order.status)}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm font-black text-nova-900">
                        {formatPriceDe(order.total)}
                      </td>
                      <td className="px-6 py-4 text-xs font-semibold text-nova-400">
            ...
            {stats.recentActivity && stats.recentActivity.length > 0 ? (
              stats.recentActivity.slice(0, activityLimit).map((log) => {
                ...
                return (
                  <div
                    key={log.id}
                    className="group flex gap-3 rounded-2xl p-3 transition-colors hover:bg-nova-50/50"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-bold text-nova-900">
                        {log.action} {log.entityType}
                      </p>
                      <p className="mt-0.5 truncate text-xs text-nova-400">ID: {log.entityId}</p>
                    </div>
