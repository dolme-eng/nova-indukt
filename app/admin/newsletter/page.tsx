    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Gérez vos abonnés et vos campagnes marketing ({subscribers.length} abonnés)</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Abonnés actifs</p>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Nouveaux (30j)</p>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Désabonnés</p>
            <h3 className="text-2xl font-black text-slate-900">{subscribers.filter(s => !s.isActive).length}</h3>
          </div>
        </div>
      </div>
                ...
                <tr key={subscriber.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-xs text-slate-500 truncate">{subscriber.firstName || "Inconnu"}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600 font-medium">
                    <div className="flex items-center gap-1.5">
                      ...
                      {format(new Date(subscriber.createdAt), "dd MMM yyyy", { locale: fr })}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    {subscriber.isActive ? (
