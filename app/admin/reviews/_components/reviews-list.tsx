  ...
  const togglePublish = async (id: string, currentStatus: boolean) => {
    ...
    try {
      const response = await fetch("/api/admin/marketing/reviews", {
        method: "PATCH",
        body: JSON.stringify({ id, isPublished: !currentStatus, action: "toggle-publish" }),
        headers: { "Content-Type": "application/json" }
      })

      if (!response.ok) throw new Error()
      
      setReviews(reviews.map(r => r.id === id ? { ...r, isPublished: !currentStatus } : r))
      toast.success(!currentStatus ? "Avis publié" : "Avis masqué")
    } catch (error) {
      ...
      <div className="grid grid-cols-1 gap-6">
              ...
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-1/4 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-slate-900 truncate">{review.user?.name || "Client Anonyme"}</p>
                      <p className="text-xs text-slate-500 truncate">{review.user?.email || "Pas d'email"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-100">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mt-0.5">Produit</p>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    {review.isVerified && (
                      <div className="flex items-center gap-1.5 text-emerald-600">
                        <ShieldCheck size={14} />
                        <span className="text-[10px] font-bold uppercase tracking-widest">Achat vérifié</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="lg:w-48 flex flex-row lg:flex-col gap-2 justify-end lg:justify-start">
                  ...
                  >
                    ...
                    {review.isPublished ? "Masquer" : "Publier"}
                  </button>
                  <button 
        ...
        {reviews.length === 0 && (
          <div className="bg-white rounded-xl border border-slate-200 p-12 text-center text-slate-500">
            ...
            <p className="font-medium text-lg">Aucun avis client à modérer.</p>
            <p className="text-sm">Les nouveaux avis apparaîtront ici pour validation.</p>
          </div>
        )}
      </div>
    </div>
